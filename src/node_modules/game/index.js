...
import {audioCache} from 'engine/resources';
...
class TestScene extends Scene {
  ...
  update(delta) {
    ...
    const holdingJump = input.keyDown('KeyW');
    if (goLeft && !goRight) {
      this.sprite.play();
    ...
    if (jump && this.player.grounded) {
      const sound = audioCache['bark'];
      const soundId = sound.play();
      const rate = 0.8 + 0.4 * Math.random();
      sound.rate(rate, soundId);
      this.player.velocity.y = -JUMP_SPEED;
    } else if (!holdingJump && !this.player.grounded && this.player.velocity.y < 0) {
